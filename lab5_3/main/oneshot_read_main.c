...
char word[64] = {0}; // Buffer to hold a whole word
int word_index = 0;

char print_morse(adc_oneshot_unit_handle_t adc_handle,
  ...
  while (1) {
    ...
    if (current_state == 0 && duration > LETTER_GAP_MS && index > 0) {
      ...
      if (letter != '\0') {
        ...
        word[word_index] = '\0'; // Null-terminate
      }

      index = 0; // Reset morse_code index
    }
    if (current_state == 0 && duration > WORD_GAP_MS && word_index > 0) {
      ESP_LOGI(TAG, "Word complete: %s", word);
      fflush(stdout);
